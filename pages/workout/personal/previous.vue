    ...
    <div v-if="EXERCISES">
      <VPageHeading div-inside button>
        <div class="workout__top">
          <VH3>Предыдущая тренировка</VH3>
        </div>
        <template v-slot:info>
          У вас есть возможность посмотреть предыдущую тренировку и выполнить
          её, если вы этого не сделали в нужный день. Тем не менее, настоятельно
          рекомендуется выполнять тренировки вовремя и не допускать отсутствия
          дней отдыха.
        </template>
      </VPageHeading>
      ...
      <VTipSmall mt="var(--space-half)" mb="var(--space-half)">
        <VP color="var(--grey-light3)">
          Не забывайте про правильное распределениe нагрузки и ваши ограничения
          по здоровью. Похудение – очень бережный процесс. Не спешите с
          нагрузками!
        </VP>
      </VTipSmall>
      <div>
...
export default {
  ...
  async asyncData({ store, redirect, app }) {
    ...
    if (
      ...
      const { data: WORKOUT } = await app.$api.Workout.getPrevious()
      return { WORKOUT }
    }
  }
}
